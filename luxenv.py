...
def get_single_observation_space():
    ...
    map_featrue_space = np.tile(np.array(list(map_featrue_names.values())).reshape(30, 1, 1), (1, 48, 48))
...
class LuxEnv(gym.Env):
    ...
    def reset_proxy(self):
        ...
        if self.kaggle_replays and EnvParam.init_from_replay_ratio != 0:
            ...
            while True:
                ...
                obs = self.proxy.reset(seed=np.random.SeedSequence().generate_state(1))
    ...
    def step(self, actions):
        ...
        obs, rewards, dones, infos = self.proxy.step(actions)  # interact with env
        self.real_obs = obs
        for player_id, player in enumerate(self.proxy.agents):
            o = obs[player]
            self.game_state[player_id] = obs_to_game_state(self.proxy.env_steps, self.env_cfg, o)
    ...
    def get_va_space(self):
        ...
        if not self.rule_based_early_step:
            space.update({'bid': spaces.MultiBinary(11), 'factory_spawn': spaces.MultiBinary([48, 48])})
        space = spaces.Dict(
            space
        )
